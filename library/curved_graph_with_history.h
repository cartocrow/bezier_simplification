...
template <class Graph>
class CurvedGraphWithHistoryAdaptor {
  public:
    using K = Inexact;
    using Vertex_handle = Graph::Vertex_handle;
    using Edge_handle = Graph::Edge_handle;
    ...
    CurvedGraphWithHistoryAdaptor(Graph& graph) : m_graph(graph) {};
    ...
    size_t num_edges() { return m_graph.num_edges(); }

    bool atPresent() { return m_undone.empty(); }
    void recallComplexity(int c) {
        while (!m_history.empty() && num_edges() < c) {
            backInTime();
        }
        while (!m_undone.empty() && num_edges() > c) {
            forwardInTime();
        }
    }
    void backInTime() {
