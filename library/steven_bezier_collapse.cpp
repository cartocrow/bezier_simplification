...
Number<Inexact> getAreaD0(Number<Inexact> a, Point<Inexact> p0, Direction<Inexact> t0, Direction<Inexact> t1, Number<Inexact> d1, Point<Inexact> p3) {
    ...
    Point<Inexact> t1p(p3 + t1.vector());
    auto t1p_ = t1p.transform(trans);
    auto t1_ = (t1p_ - Point<Inexact>(1, 0)).direction();
    auto d1_ = d1 / hyp;
    double a_ = -1 * a / (hyp * hyp);
...
/// Note that t1Dir here should *not& be the tangent direction, but instead the vector
CubicBezierCurve
